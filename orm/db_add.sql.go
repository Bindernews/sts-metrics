...
const addStrMany = `-- name: AddStrMany :many
SELECT add_str_many($1::text[])
`

func (q *Queries) AddStrMany(ctx context.Context, dollar_1 []string) ([]int32, error) {
	rows, err := q.db.Query(ctx, addStrMany, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []int32
	for rows.Next() {
		var add_str_many int32
		if err := rows.Scan(&add_str_many); err != nil {
			return nil, err
		}
		items = append(items, add_str_many)
	}
