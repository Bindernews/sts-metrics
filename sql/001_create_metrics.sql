...
CREATE FUNCTION add_str_many(s TEXT[]) RETURNS TABLE(id INT)
LANGUAGE SQL AS $$
    WITH sRows AS (SELECT * FROM unnest(s))
    INSERT INTO StrCache(str) VALUES (sRows)
    ON CONFLICT (str) DO NOTHING
    RETURNING (SELECT id FROM StrCache WHERE str = sRows);
$$;
