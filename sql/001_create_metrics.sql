...
$$;
-- Assign trigger
CREATE TRIGGER str_cache_new_insert_trg
...
CREATE TABLE RunArrays(
    ...
    -- Array of CardSpec IDs
    master_deck int array not null,
    -- Which floors potions were available as options
    potions_floor_spawned int array not null ,
    -- Which floors the player used a potion on
    potions_floor_usage int array not null ,
    -- List of relic names
    relic_ids int array not null,
    primary key (run_id)
);

CREATE VIEW RunArraysExt AS(
    SELECT a.run_id,
           array(SELECT str_cache_to_str(a.daily_mods))::text[] as daily_mods,
           array(
               SELECT s.card_full
               FROM unnest(a.master_deck) WITH ORDINALITY AS d(id, nr)
               JOIN CardSpecsEx s ON d.id = s.id
               ORDER BY d.nr
           )::text[] as master_deck,
...
CREATE TRIGGER character_list_refresh
    AFTER INSERT OR UPDATE OR DELETE ON RunsData
    FOR EACH STATEMENT EXECUTE FUNCTION character_list_refresh();
...
LANGUAGE SQL AS $$
    ...
    SELECT json_build_object(
        ...
        'master_deck', (SELECT array_to_json(arrs.master_deck) FROM arrs),
...
drop view if exists RunArraysExt;
